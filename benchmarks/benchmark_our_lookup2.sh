        ...
        for ((i=0; i<${#minimisers[@]}; i+=2)); do
          ...
          for ((j=0; j<${#thresholds[@]}; j+=2)); do
            for span in "${spans[@]}"; do
              ...
              /usr/bin/time -l -o time.txt $PROGRAM build -i "$f" -d "${BASENAME}.lookup2.dict" -k $k --m1 $m1 --t1 $t1 --m2 $m2 --t2 $t2 -s $span > prog_out22.txt 2>&1

              cat prog_out2.txt >> $LOG

              file_size=$(stat -f%z "${BASENAME}.lookup2.dict")
              ...
              for query in "${queries[@]}"; do
                  ...
                  echo $query >> $LOG
                  echo $query
                  /usr/bin/time -l -o time.txt $PROGRAM query -d "${BASENAME}.lookup2.dict" -q $query > prog_out2.txt 2>&1
