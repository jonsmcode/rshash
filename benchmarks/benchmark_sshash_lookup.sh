...
for data in $(find $DIR -mindepth 0 -maxdepth 0 -type d); do
  ...
  echo "textfile,queryfile,k,m,buildtime[s],buildmem[B],indexsize[B], space offsets[bits/kmer], space minimizers[bits/kmer], bits/key, no super kmers, space[bits/kmer], neglookuptime[ns/kmer],poslookuptime[ns/kmer]" > "$CSV"
  run $data/
done
