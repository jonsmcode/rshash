...
int RSIndex::build(const std::vector<std::vector<seqan3::dna4>> &input)
{
    ...
    for(auto & sequence : freq_skmers1) {
        ...
        for(auto && minimiser : sequence | view2) {
            ...
            if(!level_up && current_level_up) {
                ...
                skmer_positions2.push_back(length + start_position);
            }
    ...
    std::cout << "total: " << (double) (n1*offset_width+n2*offset_width+n3*offset_width+2*N+M1+M2+M3+n1+1+n2+1+n3+1+size_in_bytes(endpoints)/8+8*hashmap.size()/kmers)/kmers << "\n";
