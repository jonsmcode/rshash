...
RSIndexComp2::RSIndexComp2() : endpoints(std::vector<uint64_t>{}, 1) {}

RSIndexComp2::RSIndexComp2(
...
int RSIndexComp2::build(const std::vector<std::vector<seqan3::dna4>> &input)
{
    ...
    std::cout << "total: " << (double) (n1*offset_width+n2*offset_width+2*N+8*size_in_bytes(r1)+8*size_in_bytes(r2)+n1+1+n2+1+8*endpoints.bitCount()+64*hashmap.size())/kmers << "\n";
...
template<int level>
inline void RSIndexComp2::fill_buffer(std::vector<uint64_t> &buffer, const uint64_t mask, size_t p, size_t q)
{
...
uint64_t RSIndexComp2::streaming_query(const std::vector<seqan3::dna4> &query, uint64_t &extensions)
{
...
int RSIndexComp2::save(const std::filesystem::path &filepath) {
...
int RSIndexComp2::load(const std::filesystem::path &filepath) {
