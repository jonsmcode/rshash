...
private:
    ...
    void next_window()
    {
        ...
        const uint64_t kmerhash1 = std::min<uint64_t>((kmer_value ^ seed1) & minimiser_mask, (kmer_value_rev ^ seed1) & minimiser_mask);
        const uint64_t kmerhash2 = std::min<uint64_t>((kmer_value ^ seed2) & minimiser_mask, (kmer_value_rev ^ seed2) & minimiser_mask);
        // const uint64_t kmerhash1 = std::min<uint64_t>(murmurhash2_64::hash(kmer_value, seed1) & minimiser_mask, murmurhash2_64::hash(kmer_value_rev, seed1) & minimiser_mask);
        // const uint64_t kmerhash2 = std::min<uint64_t>(murmurhash2_64::hash(kmer_value, seed2) & minimiser_mask, murmurhash2_64::hash(kmer_value_rev, seed2) & minimiser_mask);
    ...
    void init(two_minimisers_and_window_hash_parameters const & params)
    {
        ...
        kmer_value_rev = srindex::util::crc(kmer_value, params.minimiser_size);
    ...
    bool next_minimiser_is_new()
    {
        ...
        if (uint64_t new_kmer_value = kmer_values1_in_window.back(); new_kmer_value < current.minimiser1_value) {
            ...
            minimiser1_position = kmer_values1_in_window.size() - 1u;
            new_min1 = true;
            // return true;
        }
        ...
        if (uint64_t new_kmer_value = kmer_values2_in_window.back(); new_kmer_value < current.minimiser2_value) {
            ...
            minimiser2_position = kmer_values2_in_window.size() - 1u;
            new_min2 = true;
            // return true;
        }
