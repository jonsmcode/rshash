...
uint8_t* count_minimisers(const ViewType& view, const uint8_t m_thres, const std::vector<std::vector<seqan3::dna4>> &input,
                            seqan3::contrib::sdsl::bit_vector &r, 
                            seqan3::contrib::sdsl::rank_support_v<1> &r_rank)
{
...
template <typename ViewType>
uint8_t* build_r_and_count(const ViewType& view, const uint8_t m_thres, const std::vector<std::vector<seqan3::dna4>> &input,
                            seqan3::contrib::sdsl::bit_vector &rnew,
                            seqan3::contrib::sdsl::rank_support_v<1> &rnew_rank,
                            uint8_t* count, seqan3::contrib::sdsl::rank_support_v<1> &r_rank)
{
    for(auto & sequence : input) {
        for(auto && minimiser : sequence | view) {
    ...
    rnew_rank = seqan3::contrib::sdsl::rank_support_v<1>(&rnew);
...
template <typename ViewType>
std::vector<std::vector<seqan3::dna4>> extract_frequent_parts(const ViewType& view,
...
template <typename ViewType>
uint8_t* compute_level(const ViewType& view, const uint8_t m_thres, const std::vector<std::vector<seqan3::dna4>> &input,
                        seqan3::contrib::sdsl::bit_vector &r,
                        seqan3::contrib::sdsl::rank_support_v<1> &r_rank)
{
    seqan3::contrib::sdsl::bit_vector rtmp = seqan3::contrib::sdsl::bit_vector(M, 0);
    seqan3::contrib::sdsl::rank_support_v<1> rtmp_rank;
...
void stats(const std::vector<std::vector<seqan3::dna4>> &input)
{
    ...
    seqan3::contrib::sdsl::rank_support_v<1> r1_rank;
    ...
    seqan3::contrib::sdsl::rank_support_v<1> r2_rank;
    ...
    for(int j=0; j < m_thres2; j++) {
        ...
        std::cout << "occurrences " << (j) << ": " << counter2[j] << " " << (double) counter2[j]/c2*100 << "%  cum: " << cum << " " << (double) cum/c2*100 << "% covering " << cum_skmers << ' ' << (double) cum_skmers/n*100 << "% superkmers\n";
    }
